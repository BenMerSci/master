...
x_exp_log <- expression(log(B[i] ~ N[j]))

# terrestrial
plot1 <- ggplot(data = terrestrial, aes(x = (biomass_prey *
         (biomass_predator / bodymass_mean_predator)), y = pred_flow)) +
...
plot2 <- ggplot(data = freshwater, aes(x = (biomass_prey *
         ...
         theme_bw()
# marine
plot3 <- ggplot(data = dataset, aes(x = (biomass_prey *
         (biomass_predator / bodymass_mean_predator)), y = pred_flow)) +
...
plot4 <- ggplot(data = terrestrial, aes(x = log(biomass_prey *
         (biomass_predator / bodymass_mean_predator)), y = log(pred_flow))) +
         geom_point(color = "olivedrab") +
         theme(legend.position = "none") +
...
plot5 <- ggplot(data = freshwater, aes(x = log(biomass_prey *
         ...
         theme_bw()
# log-marine
plot6 <- ggplot(data = dataset, aes(x = log(biomass_prey *
         (biomass_predator / bodymass_mean_predator)), y = log(pred_flow))) +
         geom_point(color = "deepskyblue3") +
         theme(legend.position = "none", axis.title.y = element_blank()) +
         labs(y = "", x = x_exp_log) +
...
RF_datasetsub <- dataset[ , c("pred_flow", "biomass_prey", "bodymass_mean_prey",
                  ...
                  "metabolism_predator", "ecosystem_type", "habitat_type",
                   "water_temperature", "air_temperature")]

RF_datasetsub <- RF_datasetsub |> dplyr::mutate(abundance_prey = biomass_prey / bodymass_mean_prey, abundance_predator = biomass_predator / bodymass_mean_predator)
