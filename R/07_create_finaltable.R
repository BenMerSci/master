...
dataset <- merge(interactions, traits, by.x = "prey",
           ...
           dplyr::rename(class_prey = "class", family_prey = "family",
            bodymass_prey = "bodymass", metabolism_prey = metabolic_class) |>
           dplyr::select(-"gbif_id") |>
           merge(traits, by.x = "predator", by.y = "scientific_name",
            all.x = TRUE) |>
           dplyr::rename(class_pred = "class", family_pred = "family",
            bodymass_pred = "bodymass", metabolism_pred = metabolic_class) |>
           dplyr::select(-"gbif_id") |>
           merge(enviro, by.x = "model_name", by.y = "model_name",
            all.x = TRUE) |>
           dplyr::select("prey", "predator", "pred_flow", "cons_flow",
            "total_flux", "flux_units", "biomass_prey", "bodymass_prey",
            "class_prey", "family_prey", "metabolism_prey", "degrees",
            "biomass_pred", "bodymass_pred", "class_pred", "family_pred",
            "metabolism_pred", "model_name", "model_year", "ecosystem_type",
...
dataset <- dataset[which(dataset$flux_units %in%
            c("t/km2/yr", "g/m2/yr", "g/m2/yr, dry",
            "kg/km2/yr", "kg/km2/yr, dry")), ]
...
dataset <- dataset[-which(grepl("plankton", dataset$prey) &
            grepl("plankton", dataset$predator)), ]
dataset <- dataset[-which(grepl("plankton", dataset$prey)), ]
