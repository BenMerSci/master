...
alpha <- fit4 |> tidybayes::gather_draws(alpha[pred_id])

ht <- fit4 |> tidybayes::gather_draws(ht[pred_id])
...
plot_sim_noerror(fit4, dataset)
plot_sim_error(fit4, dataset)

# Allometric relation between alpha and bodymass
alpha_bodymass <- fit4 |> tidybayes::gather_rvars(alpha[pred_id]) |>
