...
general_params <- output_stan_modeltm |>
                   tidybayes::gather_draws(mu_alpha, sd_alpha, mu_ppmr, sigma_ppmr, sigma_flow)

alpha <- output_stan_modeltm |> tidybayes::gather_draws(alpha[pred_id])
...
plot_sim_noerror(output_stan_modeltm)
plot_sim_error(output_stan_modeltm)

# Allometric relation between alpha and bodymass
alpha_bodymass <- output_stan_modeltm |> tidybayes::gather_rvars(alpha[pred_id]) |>
