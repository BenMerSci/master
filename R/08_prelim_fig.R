...
# terrestrial
plot1 <- ggplot(data = terrestrial, aes(x = ((biomass_prey / bodymass_prey) *
         (biomass_pred / bodymass_pred) * bodymass_prey), y = pred_flow)) +
         ...
         theme_bw()
# freshwater
plot2 <- ggplot(data = freshwater, aes(x = ((biomass_prey / bodymass_prey) *
         (biomass_pred / bodymass_pred) * bodymass_prey), y = pred_flow)) +
         ...
         theme_bw()
# marine
plot3 <- ggplot(data = dataset, aes(x = ((biomass_prey / bodymass_prey) *
         (biomass_pred / bodymass_pred) * bodymass_prey), y = pred_flow)) +
         ...
         ggtitle("Marin") +
         # log-terrestrial
plot4 <- ggplot(data = terrestrial, aes(x = log(((biomass_prey / bodymass_prey) *
         (biomass_pred / bodymass_pred)) * bodymass_prey), y = log(pred_flow))) +
         geom_point(color = "olivedrab") +
         theme(legend.position = "none") +
         labs(y = "log(Flux biomasse (g/m²/année))", x = x_exp_log) +
         theme_bw()
# log-freshwater
plot5 <- ggplot(data = freshwater, aes(x = log(((biomass_prey / bodymass_prey) *
         (biomass_pred / bodymass_pred)) * bodymass_prey), y = log(pred_flow))) +
         geom_point(color = "sandybrown") +
         theme(legend.position = "none", axis.title.y = element_blank()) +
         labs(y = "", x = x_exp_log) +
         theme_bw()
# log-marine
plot6 <- ggplot(data = dataset, aes(x = log(((biomass_prey / bodymass_prey) *
         (biomass_pred / bodymass_pred)) * bodymass_prey), y = log(pred_flow))) +
         geom_point(color = "deepskyblue3") +
         theme(legend.position = "none", axis.title.y = element_blank()) +
         labs(y = "", x = x_exp_log) +
