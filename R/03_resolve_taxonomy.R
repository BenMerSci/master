...
# Used traitdataform instead of taxize because the function is a wrapper round gnr_resolve
# to get the accepted name if it's a synonym
	resolved_names <- traitdataform::get_gbif_taxonomy(prelem_sp_list$original_name, resolve_synonyms = TRUE)
...
resolved_names <- cbind(resolved_names["verbatimScientificName"], resolved_names["scientificName"]) |>
		dplyr::rename(original = "verbatimScientificName", resolved = "scientificName")
...
resolved_inter_table <- base::merge(inter_table, resolved_names, by.x = "prey", by.y = "original", all.x = TRUE) |>
	dplyr::select(c("model_name","resolved","predator","pred_flow", "cons_flow", "biomass_prey","biomass_pred","degrees","total_flux")) |>
	dplyr::rename(prey = "resolved") |>
	base::merge(resolved_names, by.x = "predator", by.y = "original", all.x = TRUE) |>
	dplyr::select("model_name","prey","resolved","pred_flow", "cons_flow", "biomass_prey","biomass_pred","degrees","total_flux") |>
