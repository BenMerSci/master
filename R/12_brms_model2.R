...
curve_bf_lgnorm_rand <- bf(log(pred_flow) ~ loga + log(abund_prey) + log(bodymass_prey) + log(abund_pred),
...
lgnorm_post_rand |> 
  ggplot(aes(x = abund_prey*abund_pred*bodymass_prey, y = exp(.prediction))) + 
  stat_lineribbon() + 
  scale_fill_brewer(palette = "Oranges") +
